    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2.5">
          <div className="h-8 w-8 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600">
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge variant="outline" className={cn(
            ...
            : status === "PLACED" ? "bg-blue-50 text-blue-700 border-blue-200"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  disabled={isBusy}
                  variant="outline"
                  className="h-9 px-4 font-bold text-[11px] uppercase border-2 border-indigo-100 text-indigo-600 hover:bg-indigo-50 transition-all active:scale-95"
                >
                  {isBusy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Settings2 className="h-4 w-4 mr-2" />}
                  Change Status
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-48 rounded-2xl border-2 p-1.5 shadow-xl">
                ...
                {ORDER_STATUSES.map((status) => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      "rounded-xl font-bold text-xs uppercase px-3 py-2 cursor-pointer transition-colors",
                      order.status === status ? "bg-slate-50 text-slate-400" : "hover:bg-indigo-50 hover:text-indigo-600"
                    )}
                  >
                    {status}
                  </DropdownMenuItem>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between p-6 bg-white dark:bg-slate-900 rounded-[2rem] border-2 border-slate-100 dark:border-slate-800 shadow-sm">
        <div className="relative w-full md:w-[400px]">
          ...
          <Input 
            ...
            className="pl-10 h-11 bg-slate-50 dark:bg-slate-800/50 border-none rounded-2xl focus-visible:ring-indigo-500/30 transition-all font-medium"
          />
        </div>
        
        <div className="flex flex-wrap items-center gap-3">
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-[180px] h-11 border-2 border-slate-100 dark:border-slate-800 rounded-2xl font-bold text-xs uppercase tracking-wider">
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="group border-b last:border-0 hover:bg-slate-50/60 transition-all duration-300">
